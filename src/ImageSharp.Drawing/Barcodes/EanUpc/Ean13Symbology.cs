...
public sealed class Ean13Symbology : BarcodeSymbology
{
    ...
    /// </summary>
    private static ReadOnlySpan<int> GuardBars => new[] { 0, 1, 14, 15, 28, 29 };

    /// <inheritdoc/>
    internal override BarcodeSymbol Encode(string text, BarcodeOptions options)
    {
        string digits = EanUpcEncoder.ValidateAndApplyCheckDigit(text, 12, "EAN-13");
        ...
        // own symbol character. Digits hang one module below the digit bars and flow past the extended guard bars, as in the nominal symbol drawing.
        BarcodeTextPlacement[] placements = Array.Empty<BarcodeTextPlacement>();
        if (options.Font is not null)
        {
            float textLine = barHeight + 1;
            placements = new BarcodeTextPlacement[13];

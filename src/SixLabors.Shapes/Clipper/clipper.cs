...
namespace ClipperLib
{
  ...
  using cInt = Int32;
    ...
    public const cInt hiRange = 0x7FFF;
        ...
        public bool AddPath(Path pg, PolyType polyType, bool Closed, object src = null)
        {
#if use_lines
      ...
      if (!Closed)
        throw new ClipperException("AddPath: Open paths have been disabled.");
            ...
            for (int i = 0; i <= highI; i++) edges.Add(new TEdge() { Source = src });
    ...
    internal class Clipper : ClipperBase
    {
      ...
      public ZFillCallback ZFillFunction { get; set; }
        ...
        public bool Execute(ClipType clipType, List<PolyNode> nodes,
            PolyFillType FillType = PolyFillType.pftEvenOdd)
        {
            return Execute(clipType, nodes, FillType, FillType);
        }
        //------------------------------------------------------------------------------
           ...
           PolyFillType subjFillType, PolyFillType clipFillType)
        {
            ...
            try
            {
                succeeded = ExecuteInternal();
                //build the return polygons ...
                if (succeeded) BuildResult3(nodes);
            }
            finally
            {
                DisposeAllPolyPts();
      ...
      //------------------------------------------------------------------------------
        ...
        private OutPt AddOutPt(TEdge e, IntPoint pt)
        {
            if (e.OutIdx < 0)
            {
                ...
                outRec.Source = e.Source; // copy source from edge to outrec 
            ...
            else
            {
                ...
                if (outRec.Source != e.Source)
                {
                    // this edge was from a different/unknown source
                    outRec.Source = null; // drop source form output
                }
        ...
        private void BuildResult3(List<PolyNode> nodes)
        {
            nodes.Clear();

            //add each output polygon/contour to polytree ...
            nodes.Capacity = m_PolyOuts.Count;
            for (int i = 0; i < m_PolyOuts.Count; i++)
            {
                OutRec outRec = m_PolyOuts[i];
                int cnt = PointCount(outRec.Pts);

...
/// executes the required fallback effect. Each pass receives the preceding image as its input.
/// </para>
/// <para>
/// Shader source must define <c>fn layer_effect(position: vec2&lt;f32&gt;) -&gt; vec4&lt;f32&gt;</c>. Position uses
/// effect-local pixel-center coordinates. The return value is associated-alpha RGBA in <c>[0, 1]</c>.
/// ImageSharp supplies <c>layer_load</c>, <c>layer_load_unassociated</c>, <c>layer_sample</c>, and the declared
/// <c>imagesharp_uniforms</c> fields. Each source defines one complete shader pass.
/// </para>
/// </remarks>
public abstract class WebGPUShaderLayerEffect : LayerEffect, IWebGPUShaderEffect, IWebGPUShaderEffectSource
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="WebGPUShaderLayerEffect"/> class.
    /// </summary>
    ...
    protected WebGPUShaderLayerEffect(
        string shaderSource,
    ...
    /// Initializes a new instance of the <see cref="WebGPUShaderLayerEffect"/> class.
    /// </summary>
    /// <param name="fallbackEffect">The equivalent effect used by CPU rendering.</param>
    /// <param name="shaderSource">The complete WGSL source for each pass added by this effect.</param>
    /// <param name="uniformLayout">The named uniform values available to the shader.</param>
    protected WebGPUShaderLayerEffect(
        LayerEffect fallbackEffect,
        string shaderSource,
        WebGPUShaderUniformLayout uniformLayout)
        : base(fallbackEffect)
    {
        if (fallbackEffect is BackdropLayerEffect)
        {
            throw new ArgumentException("A backdrop effect must be wrapped by WebGPUBackdropShaderLayerEffect.", nameof(fallbackEffect));
        }

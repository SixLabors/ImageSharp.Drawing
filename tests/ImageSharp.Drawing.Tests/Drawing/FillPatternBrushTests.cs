...
{
    private static void Test(string name, Color background, Brush brush, Color[,] expectedPattern)
    {
        ...
        for (int x = 0; x < xStride; x++)
        {
            for (int y = 0; y < yStride; y++)
            {
                ...
                if (expected != actual)
                {
                    Assert.Fail($"Expected {expected} but found {actual} at ({actualX},{actualY})");
                }
            }
        }

...
public partial class ProcessWithDrawingCanvasTests
{
        ...
        where TPixel : unmanaged, IPixel<TPixel>
    {
        ...
        lowImage.Mutate(ctx => ctx.Paint(lowOptions, canvas => canvas.Fill(Brushes.Solid(Color.White), circle)));
        int lowCount = CountPixelsAbove(lowImage, 250);

        using Image<TPixel> highImage = provider.GetImage();
        highImage.Mutate(ctx => ctx.Paint(highOptions, canvas => canvas.Fill(Brushes.Solid(Color.White), circle)));
        ...
        where TPixel : unmanaged, IPixel<TPixel>
    {
        ...
        image1.Mutate(ctx => ctx.Paint(options1, canvas => canvas.Fill(Brushes.Solid(Color.White), circle)));

        using Image<TPixel> image2 = provider.GetImage();
        image2.Mutate(ctx => ctx.Paint(options2, canvas => canvas.Fill(Brushes.Solid(Color.White), circle)));
